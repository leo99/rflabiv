% RF Lab 4: Amplifier - Pre-study
% Tuomas Leinonen
% 84695P

% As the indexing/automation got rather complicated at times, 
% the code contains some "manual-tuning".
...
% Paramaters of the original design
orig = struct( ...
	'S',		[], ...
	'ABCD',		[], ...
	'Delta',	[], ...
...
% Paramaters of the stabilized design
stab = struct( ...
	'S',		[], ...
	'ABCD',		[], ...
	'Delta',	[], ...
...
xlim([-1 1]);
ylim([-1 1]);
text(0, -0.2, '\quad Output stability circles: $$|S_{22}| < 1$$ \quad', ...
	'HorizontalAlignment', 'center', ...
	'backgroundColor',	'white', ...
	'interpreter', 'latex');
...
xlim([-1 1]);
ylim([-1 1]);
text(0, -0.2, '\quad Input stability circles: $$|S_{11}| < 1$$ \quad', ...
	'HorizontalAlignment', 'center', ...
	'backgroundColor',	'white', ...
	'interpreter', 'latex');
...
% To meet specifications: SerRL = [69.8, 109]
SerRS =	[	15.3,	0,		0,		0		];
...
% Unilateral figure of merit and the error caused by unilater approach
...
legend(gca, h, ...
	{sprintf('$$G_\\mathrm{A} = %s$$ dB \\quad', Gstr(3:end)), ...
	sprintf('$$F = %s$$ dB \\qquad', Fstr(3:end)), ...
	sprintf('Input stability circle: $$|S_{11}| = %.2f$$ \\quad', abs(S(f1 == dParam.f, 1)))}, ...
	'interpreter',	'latex', ...
	'location',		'south');
...
% Constant Operating Gain Circles
figure(9);
clf;
initSmithChart(gca);
hold on;
...
legend(gca, h, ...
	{sprintf('$$G_\\mathrm{P} = %s$$ dB \\quad', Gstr(3:end)), ...
	sprintf('Output stability circle: $$|S_{22}| = %.2f$$ \\qquad', abs(S(f1 == dParam.f, 4)))}, ...
	'interpreter',	'latex', ...
	'location',		'south');
...
% Find Rs for certain F or G_A
% Would be better if different values for Rs were chosen from a 
% line connecting G_A,max and F_min points
Fi = 10^(0.737/10);
Ni = (Fi - Fm) ./ (4 * Rn) .* abs(1 + Ro).^2;
CFi = Ro ./ (1 + Ni);
RFi = sqrt(Ni.^2 + Ni .* (1 - abs(Ro).^2)) ./ (1 + Ni);
...
for Rs = z
	...
	RLi = 1 ./ (1 - Gt ./ Gp);
	RLo = 1 ./ (1 - Gt ./ Ga);
	RLi(abs(RLi) > 1e12) = Inf; % For limited accuracy
...
% SerRL = 69.8
% Rs  = -0.208 +0.516j (0.556 /_ 112.00 deg)
% Rl  = 0.356 +0.263j (0.443 /_ 36.40 deg)
% RLi = 15.01 dB (> 15)
% RLo = Inf dB (> 15)
% Ga  = 14.74 dB
% Gt  = 14.74 dB (> 13)
...
% SerRL = 109
% Rs  = -0.187 +0.443j (0.480 /_ 112.86 deg)
% Rl  = 0.487 +0.152j (0.510 /_ 17.37 deg)
% RLi = 15.01 dB (> 15)
% RLo = Inf dB (> 15)
% Ga  = 13.02 dB
% Gt  = 13.02 dB (> 13)
...
% SerRL = 93.3
% Rs  = -0.191 +0.469j (0.506 /_ 112.14 deg)
% Rl  = +0.440 +0.185j (0.478 /_ 22.81 deg)
% RLi = 15.00 dB (> 15)
% RLo = Inf dB (> 15)
% Ga  = 13.61 dB
% Gt  = 13.61 dB (> 13)
...
% Ser-RL = 102.5 
% Rs  = -0.191 +0.457j (0.495 /_ 112.72 deg)
% Rl  = +0.468 +0.166j (0.497 /_ 19.52 deg)
% RLi = 15.25 dB (> 15)
% RLo = Inf dB (> 15)
% Ga  = 13.26 dB
% Gt  = 13.26 dB (> 13)
...
% Ser-RL = 100 
% Rs   = -0.191 +0.461j (0.499 /_ 112.57 deg)
% Rin  = -0.307 -0.503j (0.589 /_ -121.45 deg)
% Rl   = +0.461 +0.171j (0.492 /_ 20.36 deg)
% Rout = +0.461 -0.171j (0.492 /_ -20.36 deg)
% RLi  = 15.22 dB (> 15)
% RLo  = Inf dB (> 15)
% Ga   = 13.35 dB
% Gt   = 13.35 dB (> 13)
